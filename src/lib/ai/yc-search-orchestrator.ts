  ...
  switch (name) {
    ...
    case "finalize_yc_search": {
      ...
      if (
        ...
        return {
          ok: false,
          errors: ["set at least one of batches, industry, tags, or a short query"],
        };
      }
      ...
      return {
        ...
        actorPreview: {
          ...
          hiringOnly: preview.hiringOnly,
          topCompaniesOnly: preview.topCompaniesOnly,
          maxRecords: preview.maxRecords,
          hitsPerPage: preview.hitsPerPage,
          requestDelay: preview.requestDelay,
        },
...
function systemPrompt(mode: "initial" | "broaden") {
  const shared = [
    "ROLE",
    "You turn one natural-language research ask into filters for the Apify actor haketa/ycombinator-companies-scraper (direct Algolia YC directory).",
    ...
    "Atlas maps finalize args into haketa input: query, batches, industries, hiringOnly, maxRecords.",
    "Haketa has NO tags[] facet — orthogonal tags (e.g. AI) are folded into query by Atlas.",
    "AND stacks shrink results. Empty batches = all seasons.",
    "Proven live runs:",
    "  industries:[\"Education\"] + batches:[] + query:\"\"  →  many Education hits (maxRecords caps)",
    "  batches:[Winter/Spring/Summer/Fall 2025] only     →  batch OR …  →  100+ (capped at maxRecords)",
    ...
    "  Education + past-2y batches + tags:[\"AI\"]         →  query becomes \"AI\" (tag fold) — only when AI was asked",
    "  tags:[\"Education\"] + industries:[]               →  BAD — use industry Education instead",
    "",
    "INFERENCE RULES",
    ...
    "Atlas expands these into haketa Apify input (statuses/regions/stages empty, hiringOnly, maxRecords, hitsPerPage 1000).",
    "",
    ...
    "APIFY:",
    '{"query":"","batches":[],"statuses":[],"industries":["Education"],"regions":[],"stages":[],"hiringOnly":false,"topCompaniesOnly":false,"maxRecords":100,"hitsPerPage":1000,"requestDelay":300}',
    "LIVE: industries Education → many hits (capped by maxRecords).",
    "",
    ...
    "APIFY batches: those four; industries:[]; query:\"\".",
    "LIVE: batch OR across those seasons → 100+ (capped).",
    "",
    ...
    "LIVE: batch OR … AND industries Education → ~10 hits.",
    "",
    ...
    "BAD: Education + 2025 batches + tags:[\"AI\"] when user did not ask for AI — over-filters.",
