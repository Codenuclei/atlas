...
function systemPrompt(mode: "initial" | "broaden") {
  const shared = [
    ...
    "   - Relative windows (\"past year\", \"last 18 months\", \"past 2 years\") → list_yc_batches_for_months with the months YOU infer.",
    ...
    "Industry: pick the single best directory industry when the ask is sector-shaped (education, fintech, healthcare, B2B, etc.).",
    "Tags: optional orthogonal facets (e.g. AI). Use tags only when they improve precision without emptying the directory. If stacking industry+tag may over-filter, prefer industry alone and widen later if needed.",
  ...
  if (mode === "broaden") {
    return [
      ...
      "- Widen the months window (e.g. 12 → 18 or 24) via list_yc_batches_for_months",
      ...
      "- Keep industry if it still matches intent; only drop industry as a last resort before empty filters",
